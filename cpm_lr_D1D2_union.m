function [CPMperformance, CPMpredictedScore, CPMmask, CPMmodel, CPMmaskPN, CPMperformancePN, CPMpredictedScorePN] = cpm_lr_D1D2_union(conMat1, conMat2, behav1, behav2, thr, var2control, subOption)
...
CPMmask1=(+(edgeCorr.r>0))-(+(edgeCorr.r<0));    CPMmask1=CPMmask1 .* (+(edgeCorr.p<=thr));    clear edgeCorr

if ~isempty(var2control)
    [edgeCorr.r, edgeCorr.p] = corr(conMat1, var2control, 'rows', 'pairwise');
    CPMmask2=(+(edgeCorr.r>0))-(+(edgeCorr.r<0));    CPMmask2=CPMmask2 .* (+(edgeCorr.p<=thr));
    CPMmask = +( (+(CPMmask1>0) + (+(sum(+(CPMmask2>0),2) >0)))>0) - (+( (+(CPMmask1<0) + (+(sum(+(CPMmask2<0),2) >0)))>0));
...
% b = ridge(behav1, conMat1(:, abs(CPMmask)>0), [0:1e-3:5e-3] );
